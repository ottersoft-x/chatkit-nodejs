...
through their own authenticated route, action, or control. The run id should
come from the `x-chatkit-run-id` response header or another app-owned durable
run record.
...
1. Start a test backend that logs request method, path, headers, body, response
   headers, stream lifecycle events, and calls to app-owned cancel or attach
   actions.
...
8. If stop only closes the active stream, keep backend cancellation behind an
   explicit app-owned cancel action/control and continue treating stream close as
   subscriber detach.
